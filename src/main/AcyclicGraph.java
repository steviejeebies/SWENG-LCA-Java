    ...
    int [] findLCA(int [] args) {
        ...
        for(int i = 0; i < sizeGraph; i++) {
            if(shareAncestor(args, i)) {
                if(vertices[i].numAncestors < distanceFromRoot) continue;   // this ancestor is closer to root that what we have, we can ignore it
                if(vertices[i].numAncestors > distanceFromRoot) potentialLCAs = new LinkedList<>();    // clear list of current LCAs as they are closer to root
